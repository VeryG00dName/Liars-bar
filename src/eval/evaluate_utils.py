...
def evaluate_agents(env, device, players_in_this_game, episodes=11, is_tournament=False, two_player=None, track_experts=False, progress_callback=None, cheat_expert_index=None):
    ...
    # Track belief policies and models
    belief_models = {}
    belief_spaces = {}
    ...
    # Initialize belief tracking for each belief-based agent
    for agent_name in env.possible_agents:
        pid = agent_to_player.get(agent_name)
        if pid and players_in_this_game[pid].get('is_belief_space_policy', False):
            player_data = players_in_this_game[pid]
            belief_models[agent_name] = player_data.get('belief_model')
            
            # Initialize belief space as uniform distribution per opponent
            belief_spaces[agent_name] = {}
            num_opponent_types = player_data.get('num_opponent_types', 10)
            
            for opp_agent in env.possible_agents:
                if opp_agent != agent_name:
                    belief_spaces[agent_name][opp_agent] = np.ones(num_opponent_types) / num_opponent_types
    ...
    with torch.no_grad():
        for game_idx in range(1, episodes + 1):
            ...
            while env.agent_selection is not None:
                        ...
                        if opp_agent != agent and opp_agent in last_actions and last_actions[opp_agent] is not None:
                            ...
                            if belief_models.get(agent) is not None:
                                ...
                                with torch.no_grad():
                                    ...
                                    belief_spaces[agent][opp_agent] = updated_belief.squeeze().cpu().numpy()
                            else:
                                # Use Bayesian update
                                belief_spaces[agent][opp_agent] = bayesian_belief_update(
                                    belief_spaces[agent][opp_agent],
                                    obs_new, obs_old, action, action_mask, 
                ...
                elif player_data.get('is_belief_space_policy', False):
                ...
                elif player_data.get('is_newer_obs_model', False):
                    # [keep existing newer obs model code]
                    # ... 
                    pass
                elif player_data.get('is_stacked_model', False):
                    # [keep existing stacked model code]
                    # ...
                    pass
                else:
                    # [keep existing standard model code]
                    # ...
                    pass
                
                # Store current action and observation for future belief updates
                last_actions[agent] = action
                if 'observation' in locals():
                    # Get both observation formats for future belief updates
                    observation_dict_new = env.observe(agent, new=True)
                    last_observations_new[agent] = observation_dict_new[agent]
                    observation_dict_old = env.observe(agent, new=False)
                    last_observations_old[agent] = observation_dict_old[agent]
