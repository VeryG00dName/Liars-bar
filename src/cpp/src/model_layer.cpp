...
attention_block(
    ...
    auto pol = policy_indices.to(x.device()).to(torch::kLong).contiguous();
    const int64_t B = x.size(0), T = x.size(1), head_dim = hidden_dim / num_heads;
    std::unordered_map<std::string, std::chrono::microseconds> dummy_timers;

    std::string prefix = "transformer.layers." + std::to_string(layer_idx);
    auto in_proj_weight = get_weight(batched_weights, prefix + ".self_attn.in_proj_weight");
    auto in_proj_bias = get_weight(batched_weights, prefix + ".self_attn.in_proj_bias");
    int64_t weight_chunk_dim = (in_proj_weight.dim() == 3) ? 1 : 0;
    int64_t bias_chunk_dim = (in_proj_bias.dim() == 2) ? 1 : 0;
    auto qkv_weights = in_proj_weight.chunk(3, weight_chunk_dim);
    auto qkv_biases = in_proj_bias.chunk(3, bias_chunk_dim);

    auto q = lb::kernels::indexed_batched_linear(x, qkv_weights[0], qkv_biases[0], pol, dummy_timers);
    auto k = lb::kernels::indexed_batched_linear(x, qkv_weights[1], qkv_biases[1], pol, dummy_timers);
    auto v = lb::kernels::indexed_batched_linear(x, qkv_weights[2], qkv_biases[2], pol, dummy_timers);
    ...
    auto attn_output = torch::scaled_dot_product_attention(q, k, v, torch::nullopt, 0.0, true);
    attn_output = attn_output.transpose(1, 2).contiguous().view({B, T, hidden_dim});
    attn_output = lb::kernels::indexed_batched_linear(attn_output,
        get_weight(batched_weights, prefix + ".self_attn.out_proj.weight"),
        get_weight(batched_weights, prefix + ".self_attn.out_proj.bias"), pol, dummy_timers);

    auto residual = x + attn_output;
    auto post_attn = lb::kernels::indexed_batched_layer_norm(residual,
        get_weight(batched_weights, prefix + ".norm1.weight"),
        get_weight(batched_weights, prefix + ".norm1.bias"), pol);
...
moe_block(
    ...
    auto gate_weight = get_weight(batched_weights, prefix + ".moe.gate.weight");
    auto gate_bias = get_weight(batched_weights, prefix + ".moe.gate.bias");
    auto w1_all = get_weight(batched_weights, prefix + ".moe.experts.w1");
    auto w2_all = get_weight(batched_weights, prefix + ".moe.experts.w2");
    auto b1_all = get_weight(batched_weights, prefix + ".moe.experts.b1");
    auto b2_all = get_weight(batched_weights, prefix + ".moe.experts.b2");

    auto gate_logits = lb::kernels::indexed_batched_linear(x, gate_weight, gate_bias, pol, dummy_timers);
    auto probs = torch::softmax(gate_logits, -1);
    auto topk_vals_idx = torch::topk(gate_logits, top_k, -1);
    auto topk_indices = std::get<1>(topk_vals_idx);
    auto topk_scores = torch::gather(probs, -1, topk_indices);
    ...
    out.insert("topk_scores", topk_weights);
    out.insert("moe_output", moe_output);
    return out;
}
...
moe_routing_sort(
    ...
    auto gate_logits = lb::kernels::indexed_batched_linear(x,
        get_weight(batched_weights, prefix + ".moe.gate.weight"),
        get_weight(batched_weights, prefix + ".moe.gate.bias"), pol, dummy_timers);
    ...
    int64_t num_policies_for_key = get_weight(batched_weights, prefix + ".moe.experts.w1").size(0);
    auto combined_key = flat_expert_indices * num_policies_for_key + flat_policy_indices;
    auto sort_order = torch::argsort(combined_key);
    auto sorted_expert_indices = flat_expert_indices.index_select(0, sort_order);
    auto sorted_token_indices = expanded_token_indices.index_select(0, sort_order);
...
torch::Tensor moe_group_ranges(
    ...
    auto se_cpu = sorted_expert_indices.to(torch::kCPU);
    auto sp_cpu = sorted_policy_indices.to(torch::kCPU);
    const auto* se = se_cpu.data_ptr<int64_t>();
    const auto* sp = sp_cpu.data_ptr<int64_t>();
    const int64_t N = se_cpu.size(0);
    ...
    while (cursor < N) {
        ...
        while (end < N && se[end] == expert_id && sp[end] == policy_id) ++end;
        rows.push_back(cursor);
        rows.push_back(end - cursor);
