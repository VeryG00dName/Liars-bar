...
std::vector<uint8_t> EvalManager::run_model(int policy_id,
    ...
    auto batch = prepare_inference_batch(requests, kInferenceDevice);
    ...
    const int64_t batch_size = action_logits.size(0);
    ...
    auto opts_long_device = torch::TensorOptions().dtype(torch::kLong).device(kInferenceDevice);
    ...
    auto has_legal = final_masks.any(1);
    if (!has_legal.all().item<bool>()) {
        auto fallback_rows = has_legal.logical_not().nonzero().flatten();
        auto final_masks_cpu = final_masks.to(torch::kCPU);
        auto final_logits_cpu = final_logits.to(torch::kCPU);
        for (int64_t i = 0; i < fallback_rows.size(0); ++i) {
            const int64_t row = fallback_rows[i].item<int64_t>();
            auto mask_row = final_masks_cpu.select(0, row);
            mask_row.fill_(false);
            bool assigned = false;
            const auto& req = requests[static_cast<size_t>(row)];
            for (int action = 0; action < 7; ++action) {
                if (req.mask[action]) {
                    mask_row[action] = true;
                    assigned = true;
                }
            }
            if (!assigned) {
            ...
            final_logits_cpu.select(0, row).fill_(0.0f);
        }
        final_masks = final_masks_cpu.to(kInferenceDevice, false, true);
        final_logits = final_logits_cpu.to(kInferenceDevice, false, true);
    }
    ...
    std::vector<uint8_t> chosen(actions_ptr, actions_ptr + actions_cpu.numel());
    return chosen;
}
