...
class PlayVsAIGUI:
    ...
    def __init__(self, root):
        ...
        # Track moves made since the last time it was YOUR turn. Includes your move.
        self.moves_since_last_turn = []  # list of dicts
        ...
        self.challenge_banner_until: Optional[datetime] = None
    ...
    def create_checkpoint_panel(self):
        ...
        # Show the actual configured checkpoint path, portable separators
        checkpoint_display = self.CHECKPOINT_RELATIVE_PATH
    ...
    def create_control_buttons(self):
        ...
        delay_spin = ttk.Spinbox(
            delay_frame, from_=0, to=5000, increment=250, textvariable=self.ai_delay_ms_var, width=7
        )
    ...
    def create_action_window(self):
        ...
        for action_value, label in actions:
            btn = ttk.Button(self.action_window, text=label, command=lambda val=action_value: self.select_action(val))
    ...
    def _do_ai_move(self, agent_id):
        ...
        try:
            ...
            # Log the move (we keep only the 4 most recent, incl. yours)
            self.track_move(agent_id, action)

            # Trigger the left-column CHALLENGE box when appropriate
            self._maybe_show_challenge_banner(env, agent_id, action)
    ...
    def _maybe_show_challenge_banner(self, env, acting_agent_id: str, action: int):
        """Show a temporary CHALLENGE box in the left column when an AI plays action 6
        against the human's previous action (i.e., when the human was the last actor).

        Color derived from env.last_action_bluff: green if False, red if True.
        Visible for 5 seconds, then cleared by render cycle.
        """
        try:
            if int(action) == 6 and acting_agent_id in self.ai_agents:
                last_agent = getattr(env, "last_action_agent", None)
                if last_agent == self.human_agent_id:
                    bluff = bool(getattr(env, "last_action_bluff", False))
                    self.challenge_banner_until = datetime.now() + timedelta(seconds=5)
                    self.challenge_banner_color = "red" if bluff else "green"
                    self.challenge_banner_text = "CHALLENGE"
                    self.challenge_banner_by = acting_agent_id
        except Exception:
            # Never let UI decoration crash the loop
            pass
    ...
    def _check_human_elimination_and_finish(self):
        ...
        everyone_out = all(env.terminations.get(a, False) or env.truncations.get(a, False) for a in env.possible_agents)
    ...
    def start_game(self):
        ...
        self.human_agent_id = (
            "player_0" if "player_0" in self.current_env.possible_agents else self.current_env.possible_agents[0]
        )
    ...
    def _final_rewards(self, env):
        """Safely get totals at game end."""
        # Prefer cumulative totals (AEC), else fall back to per-step rewards dict
        totals = getattr(env, "cumulative_rewards", None)
        if isinstance(totals, dict) and len(totals) == len(env.possible_agents):
            return totals
        return env.rewards

    def show_game_result(self):
        env = self.current_env
        rewards = self._final_rewards(env)

        # Resolve winner robustly
        winner = getattr(env, "winner", None)
        if not winner:
            for ag, info in getattr(env, "infos", {}).items():
                if info.get("winner"):
                    winner = ag
                    break

        # Build message
        result_lines = ["Game Results:"]
        for agent in env.possible_agents:
            result_lines.append(f"{agent}: {rewards.get(agent, 0)}")

        if winner:
            result_lines.append("")
            result_lines.append("Winner(s):")
            result_lines.append(winner)
        else:
            # Fallback if no explicit winner flag
            max_reward = max(rewards.values()) if rewards else 0
            winners = [a for a, r in rewards.items() if r == max_reward]
            result_lines.append("")
            result_lines.append("Winner(s):")
            result_lines.extend(winners or ["(could not determine)"])

        messagebox.showinfo("Game Over", "\n".join(result_lines))
        self.current_env = None

    def track_move(self, agent, action):
        """Record a move (ordered & timestamped). Includes YOUR move at start of the cycle."""
        try:
            a = int(action)
        except Exception:
            a = action
        ...
        if isinstance(a, int) and a in (0, 1, 2):
            num_cards = a + 1  # 0,1,2 -> 1,2,3
            text = f"played {num_cards} card{'s' if num_cards > 1 else ''}"
        elif isinstance(a, int) and a in (3, 4, 5):
            num_cards = a - 2  # 3,4,5 -> 1,2,3
            text = f"played {num_cards} card{'s' if num_cards > 1 else ''}"
        ...
        # Keep only the most recent 4 entries (incl. yours)
        MAX_ENTRIES = 4
        if len(self.moves_since_last_turn) > MAX_ENTRIES:
            self.moves_since_last_turn = self.moves_since_last_turn[-MAX_ENTRIES:]

    def _moves_panel(self):
        if self.moves_since_last_turn:
            moves_lines = [f"{m['idx']}) {m['time']}  {m['agent']} {m['text']}" for m in self.moves_since_last_turn]
            moves_text = "\n".join(moves_lines)
        else:
            moves_text = "No moves in this cycle."
        # Rename to make it clear your move is included
        return Panel(moves_text, title="Recent Moves (incl. yours)", border_style="blue")
    ...
    def render_player_mode(self, env):
        ...
        # Left column: fixed-height table-card panel + a filler box to take the remaining space
        table_card_panel = Panel(
            f"Table Card: [bold yellow]{env.table_card}[/bold yellow]",
            title="Table Card",
            border_style="bright_blue",
        )
        left_filler = Panel("", title="", border_style="bright_blue")
        ...
        # Moves (kept to last 4, incl. yours)
        moves_panel = self._moves_panel()
        ...
        for agent in env.possible_agents:
            ...
            elif getattr(env, "round_eliminated", {}).get(agent, False):
        ...
        # No top banner; challenge feedback is rendered in the left column.
        layout.split(
        ...
        # Left column: small fixed-height table-card, plus a CHALLENGE box that fills remaining height
        # Challenge panel content (active or placeholder)
        active = self.challenge_banner_until is not None and datetime.now() <= self.challenge_banner_until
        if not active:
            # Clear expired state so it doesn't stick around
            self.challenge_banner_until = None
            self.challenge_banner_text = ""
            self.challenge_banner_by = None
        challenge_body = self.challenge_banner_text if active else ""
        challenge_style = self.challenge_banner_color if active else "grey50"
        challenge_title = "Challenge" if not active else f"Challenge by {self.challenge_banner_by}"
        challenge_panel = Panel(challenge_body, title=challenge_title, border_style=challenge_style)

        layout["left"].split(
            Layout(table_card_panel, name="table_card", size=3),
            Layout(challenge_panel, name="challenge_box", ratio=1),
            Layout(left_filler, name="left_filler", ratio=1)
        )

        # Center column: 3 stacked blocks. Give the moves panel enough height to show 4 entries comfortably.
        layout["center"].split(
            Layout(your_hand_panel, name="your_hand", size=7),
            Layout(opponent_table, name="opponent_hands", size=7),
            Layout(moves_panel, name="moves", size=8),  # enlarged from 5 -> 8
        )
