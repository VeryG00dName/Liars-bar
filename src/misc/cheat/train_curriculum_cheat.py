...
historical_label_mapping = {}  # This can be populated if needed
...
def train_curriculum(env, device, num_episodes=10000, load_checkpoint=False, load_directory=None, log_tensorboard=True):
    ...
    shared_policy_net = PolicyNetwork(
            ...
            num_experts=11,  # one expert per injected bot
            use_lstm=True,
            use_dropout=True,
            use_layer_norm=True,
        ).to(device)
    ...
    # Extend RolloutMemory to store expert indices
    ...
    for episode in range(start_episode, num_episodes + 1):
        ...
        while env.agent_selection is not None:
            ...
            # -------------------- NEW: Select Expert Directly Using Opponent Label --------------------
            # Compute expert_index based on the current opponent at data collection time.
            expert_index = HARD_CODED_LABELS.get(current_opponent_name, historical_label_mapping.get(current_opponent_name, -1))
            #logger.info(f"Agent {agent} selected expert {expert_index} for opponent {current_opponent_name}")
            # -------------------------------------------------------------------------------------
            
            # -------------------- Action Selection --------------------
            if agent in training_agents:
                observation_tensor = torch.tensor(final_obs, dtype=torch.float32, device=device).unsqueeze(0)
                # Pass the computed expert_index into the policy network.
                ...
                if current_opponent_type == "hardcoded":
                    ...
                    for opp in env.possible_agents:
                        if opp != agent:
                            memory_full = query_opponent_memory_full(agent, opp)
                            # Use the helper to convert the memory to a features list
                            features_list = convert_memory_to_features(memory_full, response2idx, action2idx)
                            if features_list:
                                feature_tensor = torch.tensor(features_list, dtype=torch.float32, device=device).unsqueeze(0)
                                with torch.no_grad():
                                    projected = event_encoder(feature_tensor)
                                    strategy_embedding, _ = strategy_transformer(projected)
                                # Take only the first 5 dimensions for this opponent
                                hist_emb = strategy_embedding.cpu().detach().numpy().flatten()[:5]
                            else:
                                hist_emb = np.zeros(5, dtype=np.float32)
                            historical_embeddings.append(hist_emb)
                    # Concatenate historical embeddings (should be 5 dims per opponent, 10 in total for 2 opponents)
                    historical_embedding_concat = np.concatenate(historical_embeddings, axis=0)
                    # Combine the original 16-dim observation with the 10-dim historical embeddings to form a 26-dim input.
                    extended_obs = np.concatenate([final_obs, historical_embedding_concat], axis=0)
                    observation_tensor = torch.tensor(extended_obs, dtype=torch.float32, device=device).unsqueeze(0)
                    with torch.no_grad():
            ...
            for ag in agents:
                if ag != agent:
                    ...
                    if ag in training_agents:
                        # Store the expert_index with the transition so that later updates use the same label.
        ...
        if episode % config.UPDATE_STEPS == 0:
            for agent in training_agents:
                ...
                # Use the stored expert indices from memory.
                stored_expert_indices = memory.expert_inputs[agent]
                if len(stored_expert_indices) > 0:
                    # Choose the mode (most frequent expert index) from the collected data.
                    expert_index = np.bincount(np.array(stored_expert_indices, dtype=np.int64)).argmax()
                else:
                    expert_index = HARD_CODED_LABELS.get(current_opponent_name, historical_label_mapping.get(current_opponent_name, -1))
                
                for _ in range(config.K_EPOCHS):
                    # Forward pass using the stored expert index.
