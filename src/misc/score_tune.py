...
def train_new_agents(env, device, num_episodes=20000, log_tensorboard=False):
    ...
    # Train the agents using the training script.
    results = train(
...
def prune_low_ranking_agents(trial_agents: dict, openskill_model, max_agents: int = MAX_AGENTS):
    ...
    sorted_agents = sorted(all_agents, key=lambda x: x[1].mu, reverse=True)
...
def objective(trial: optuna.trial.Trial) -> float:
    """
    Train a new 3-agent team for the current trial using tuned scoring parameters,
    run a Swiss tournament with all trials' agents, set the current trial's score based
    on its performance, and return the current trial's score.
    """
    global trial_agents
    ...
    # === 2) Train new 3-agent team with these scoring parameters ===
    train_env = LiarsDeckEnv(num_players=3, render_mode=config.RENDER_MODE, scoring_params=scoring_params)
    ...
    # === 9) Return the current trial's score ===
...
def main():
    ...
    logger.info("Starting Swiss-based tournament tuning (scoring parameters) with OpenSkill integration...")
