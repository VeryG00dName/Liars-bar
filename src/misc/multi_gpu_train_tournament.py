...
def average_state_dicts(state_dicts):
    """
    Averages a list of state_dicts (each a dict of tensors) by summing and dividing by the number of dicts.
    Assumes that each state dict has the same keys.
    """
    avg_state = {}
    # Loop over keys from the first state dict.
    for key in state_dicts[0].keys():
        # Initialize accumulator using a clone of the first tensor converted to float.
...
def train_group_process(group, agent_group_state, block_episode_offset, gpu_id, obp_state):
    ...
    Logs are printed to standard output so you can see progress.
    """
    # Set up logger to stream to standard output.
    logger = logging.getLogger(f"TrainGroup-{gpu_id}")
    logger.setLevel(logging.INFO)
    
    # Ensure only one handler is added (avoid duplicate logs)
    ...
    try:
        ...
        train(
            ...
            writer=None,
            logger=logger,  # Pass our logger so that training progress gets logged.
        ...
        # Move agents back to CPU.
        for pid, agent in agent_group_state.items():
            move_agent_to_device(agent, 'cpu')
        
        # Move OBP model to CPU and get its state.
        ...
        # Return the updated agents and OBP model state.
...
def main():
    ...
    # The device for operations that are not parallelized (e.g., OBP initialization, checkpointing)
    ...
    try:
        for batch_id in range(start_batch, 20):
            ...
            # --- Run group trainings concurrently on multiple GPUs ---
            available_gpus = list(range(torch.cuda.device_count()))
            if not available_gpus:
                available_gpus = [0]  # Fallback in case no GPUs are detected.
            futures = []
            # Using ProcessPoolExecutor so that each group can run on its assigned GPU.
            with ProcessPoolExecutor(max_workers=len(available_gpus)) as executor:
                for group_idx, group in enumerate(groups):
                    ...
                    # Deep-copy the agents for this group to send to the subprocess.
                    agent_group_state = {pid: copy.deepcopy(player_pool[pid]) for pid in group}
                    
                    # Submit the training job for this group.
