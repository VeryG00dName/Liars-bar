...
def objective(trial: optuna.trial.Trial) -> float:
    """
    Objective function for tuning learning hyperparameters.
    The function samples the learning hyperparameters, sets them in the config,
    creates an environment, runs the training routine (tune_train) that returns the best win rate,
    and then returns that win rate as the trial’s score.
    """
    # --- 1) Sample learning hyperparameters using Optuna ---
    # Set config values for learning hyperparameters before training
    config.LEARNING_RATE = trial.suggest_float("LEARNING_RATE", 0.00005, 0.001, log=True)
    config.GAMMA = trial.suggest_float("GAMMA", 0.95, 0.999)
    config.EPS_CLIP = trial.suggest_categorical("EPS_CLIP", [0.05, 0.2, 0.3])
    config.K_EPOCHS = trial.suggest_categorical("K_EPOCHS", [2, 8, 10])
    config.INIT_ENTROPY_COEF = trial.suggest_categorical("INIT_ENTROPY_COEF", [0.005, 0.02, 0.05])
    config.GAE_LAMBDA = trial.suggest_categorical("GAE_LAMBDA", [0.9, 0.98])
    ...
    hyperparams = {
        ...
        "INIT_ENTROPY_COEF": config.INIT_ENTROPY_COEF,
        "GAE_LAMBDA": config.GAE_LAMBDA,
    }
    logging.info(f"Trial {trial.number} learning hyperparameters: {hyperparams}")
    ...
    writer.add_text("Learning_Hyperparameters", param_str, global_step=trial.number)
    
    # --- 3) Create training environment ---
    # Note: If LiarsDeckEnv still requires scoring_params, you can pass the default or existing ones.
    train_env = LiarsDeckEnv(num_players=3)
    ...
    # --- 4) Run the tune_train training routine, passing the trial object ---
    ...
    trial_results[trial.number] = {
        "learning_hyperparameters": hyperparams,
...
def main():
    ...
    logger = logging.getLogger("HyperTune")
    logger.info("Starting tuning of learning hyperparameters using win rate from training...")

    storage_name = "sqlite:///my_optuna_study.db"
    study_name = "liars_deck_learning_hyperparams_study"
    ...
    # --- Note: Removed enqueue_trial with DEFAULT_SCORING_PARAMS ---
    N_TRIALS = 50
    ...
    logger.info("Learning hyperparameter tuning completed!")
    if study.best_value is not None:
        logger.info(f"Best Score: {study.best_value*100:.2f}%")
        logger.info("Best Learning Hyperparameters:")
